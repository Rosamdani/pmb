...
if (isset($_POST['submit-login'])) {
    ...
    $_SESSION['errorCaptchaLogin'] = "";
    $_SESSION['errorEmailLogin'] = "";
    $_SESSION['errorPassLogin'] = "";
    $email = $_POST['email'];
    $password = md5($_POST['password']);

    if ($_POST['captcha'] != $_SESSION['captcha_code']) {
        $valid = false;
        $_SESSION['errorCaptchaLogin'] = "*Captcha salah!";
    }

    $query1 = mysqli_query($con, "SELECT * FROM users");
    if ($query1->num_rows > 0) {
        while ($row = mysqli_fetch_array($query1)) {
            if ($email == $row['email'] && $password == $row['password'] && $valid == true) {
                $_SESSION['id_user'] = $row['id_user'];
                $_SESSION['kelas'] = $rosw['kelas'];
                if($row['kelas'] == "admin"){
                    $_SESSION['admin'] = true;
    ...
    $_SESSION['errorEmailLogin'] = "*Email atau password salah!";
...
<body class="box-border bg-slate-100">
    ...
    <section class="w-[96%] mx-auto bg-white flex box-border">
        <div class="w-[50%] border-r-2 border-slate-200 box-border bg-slate-200">
            ...
            <form method="POST" class="px-4 py-2 text-slate-500 bg-white">
                <div class="container h-10 rounded-sm border border-slate-300 overflow-hidden flex mt-4">
                    ...
                    <input required type="email" placeholder="Email" name="email" class="w-full outline-emerald-300 px-3">
                </div>
                <p class="text-red-400"><?= @$_SESSION['errorEmailLogin'] ?></p>
                <div class="container h-10 rounded-sm border border-slate-300 overflow-hidden flex mt-4">
                    <div class="w-12 h-full flex items-center justify-center bg-slate-200">
                ...
                <div class="container h-10 rounded-sm overflow-hidden flex mt-4 justify-between">
                    ...
                    <a href="login.php" class="h-full w-14 rounded-sm bg-emerald-400 flex items-center justify-center text-white"><i class="fa-solid fa-arrows-rotate"></i></a>
                </div>
                <div class="container h-10 rounded-sm border border-slate-300 overflow-hidden flex mt-4">
                    <div class="w-12 h-full flex items-center justify-center bg-slate-200">
                ...
                <p class="text-red-400"><?= @$_SESSION['errorCaptchaLogin'] ?></p>
                <div class="container h-10 rounded-sm overflow-hidden flex mt-4">
                    <button name="submit-login" class="w-24 hover:bg-emerald-600 h-full bg-emerald-500 rounded text-white font-semibold">
                        Login
                    </button>
                </div>
                <div class="container h-10 rounded-sm overflow-hidden justify-center items-center flex mt-4">
                    <p>Belum Punya akun?<a href="registrasi.php" class="text-emerald-500 font-semibold">&nbsp; Registrasi!</a></p>
                </div>
            </form>
        ...
        <div class="container justify-between flex px-10">
            ...
            <ul class="mr-10">
                ...
                <li class="mb-3 text-white font-light">
                    ...
                    <p>Novia : (+62) 8929350913</p>
                    <p>Herdian : (+62) 093128921</p>
                    <p>Fikri : (+62) 853634646</p>
                </li>
            </ul>
        </div>
        <div class="container flex h-10 justify-between bg-slate-700 items-center px-5">
        <p class="text-white">&copy;Rosyam, <?= date("Y") ?>. All Right Reserved</p>
